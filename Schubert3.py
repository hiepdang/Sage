"""
Schubert3 is a Sage package which supports computation in Intersection Theory on
smooth varieties. It is written by Python programming language.

It deals with varieties, vector bundles on varieties, equivariant vector bundles
on varieties with torus actions and morphisms between varieties. 

In Schubert3, a variety is given by the dimenion, variables, degrees, and 
relations such that we can determine its Chow ring. 
For example, an n-dimensional projective space P^n is given by dimension n, one 
variable h with deg(h) = 1 and one relation h^{n+1}, where h is the hyperplane 
class of P^n. A Grassmannian G(k,n) is given by dimension k(n-k), n-k variables 
q_1, ..., q_{n-k} with deg(q_i) = i and n-k relations. 

There are two ways to give a vector bundle on an abstract variety Schubert3. 
...
this vector bundle. The second one is represented by only an element of the Chow
 ring which is the Chern character of this vector bundle. 

 The idea for implementation of vector bundles is based on the splitting principle.
A vector bundle on a variety is given by the rank and the Chern classes or the 
Chern character. The idea for implementation of vector bundles is based on the 
splitting principle. 

An equivariant vector bundle on a variety with the torus action is given by its 
weights of the torus action on the ordinary vector bundle.

A morphism between two varieties is given by its pullback which is a 
homomorphism of the corresponding Chow rings.

In order to use Bott's formula for computing the intersection numbers, we need 
to implement some new computational odjects such as the equivariant vector 
bundles on the varieties with the torus action and the moduli spaces of stable 
maps. An equivariant vector bundle is given by its weights of the torus action 
on the ordinary vector bundle. 

A moduli space of stable maps is given by a projective space and a positive 
integer which is the degree of the stable maps. 

We also implemented a combinatorial object which allows us to present the fixed 
point components of the torus action on the moduli spaces of stable maps.
"""
...
class Variety(SageObject):
    ...
    def O(self, d):
        ...
        EXAMPLES::
            ...
            2*h + 1
        """
        return VectorBundle(self, rank=1, chern_class=1+d*self._var[0])
...
class Grassmannian(Variety):
    ...
    def poincare_polynomial(self, var=None):
        """
    ...
    def fixed_points(self):
        ...
        this Grassmannian G(k,n) with weights 0, 1, 2, ..., n-1.
...
class EquivariantVectorBundle(SageObject):
    ...
    def _repr_(self):
        ...
        w = self._weights
        V = self._variety
        return "An equivariant vector bundle of rank %s on %s" %(len(w),V)
...
def linear_subspace(k,d,n):
    """
    Compute the number of linear subspaces on a general hypersurface
    """
    if d < 3:
...
def linear_subspaces_hypersurface(k,d,n):
    """
    EXAMPLES:

        sage: linear_subspaces_hypersurface(2,4,7)
        3297280
        sage: linear_subspaces_hypersurface(2,7,14)
        279101475496912988004267637
        sage: linear_subspaces_hypersurface(3,3,8)
        321489
    """
    if d < 3:
        raise ValueError("This must be satisfied")
    ...
    def fixed_points(self):
        ...
        for i in S:
            for j in S.difference({i}):
                R.append([Graph1(D[0],i,j), 2*self._d])
        if self._d == 2:
            for i in S:
                for j in S.difference({i}):
                    for k in S.difference({j}):
                        R.append([Graph2(D[1],i,j,k),2])
        if self._d == 3:
            for i in S:
                for j in S.difference({i}):
                    for k in S.difference({j}):
                        ...
                        for h in S:
                            if h != k:
                                R.append([Graph31(D[2],i,j,k,h),2])
                            if h != j:
                                R.append([Graph32(D[2],i,j,k,h),6])
        if self._d == 4:
            for i in S:
                for j in S.difference({i}):
                    for k in S.difference({j}):
                        R.append([Graph2(D[1],i,j,k),3])
                        R.append([Graph2(D[2],i,j,k),8])
                        for h in S:
                            if k != h:
                                R.append([Graph31(D[3],i,j,k,h),2])
                                R.append([Graph31([1,2,1],i,j,k,h),4])
                            if j != h:
                                R.append([Graph32(D[3],i,j,k,h),4])
                            for m in S:
                                if k != h and h != m:
                                    R.append([Graph41(D[4],i,j,k,h,m),2])
                                if k != h and k != m:
                                    R.append([Graph42(D[4],i,j,k,h,m),2])
                                if j != h and j != m:
                                    R.append([Graph43(D[4],i,j,k,h,m),24])
        if self._d == 5:
            for i in S:
                for j in S.difference({i}):
                    for k in S.difference({j}):
                        R.append([Graph2([4,1],i,j,k),4])
                        R.append([Graph2([3,2],i,j,k),6])
                        for h in S:
                            if k != h:
                                R.append([Graph31([3,1,1],i,j,k,h),3])
                                R.append([Graph31([1,3,1],i,j,k,h),6])
                                R.append([Graph31([2,2,1],i,j,k,h),4])
                                R.append([Graph31([2,1,2],i,j,k,h),8])
                            if j != h:
                                R.append([Graph32([3,1,1],i,j,k,h),6])
                                R.append([Graph32([2,2,1],i,j,k,h),8])
                            for m in S:
                                if k != h and h != m:
                                    R.append([Graph41([2,1,1,1],i,j,k,h,m),2])
                                    R.append([Graph41([1,2,1,1],i,j,k,h,m),2])
                                if k != h and k != m:
                                    R.append([Graph42([2,1,1,1],i,j,k,h,m),4])
                                    R.append([Graph42([1,2,1,1],i,j,k,h,m),4])
                                    R.append([Graph42([1,1,2,1],i,j,k,h,m),2])
                                if j != h and j != m:
                                    R.append([Graph43([2,1,1,1],i,j,k,h,m),12])
                                for n in S:
                                    if k != h and h != m and m != n:
                                        R.append([Graph51([1,1,1,1,1],i,j,k,h,m,n),2])
                                    if k != h and h != m and h != n:
                                        R.append([Graph52([1,1,1,1,1],i,j,k,h,m,n),2])
                                    if k != h and k != m and k != n:
                                        R.append([Graph53([1,1,1,1,1],i,j,k,h,m,n),6])
                                    if j != h and h != m and h != n:
                                        R.append([Graph54([1,1,1,1,1],i,j,k,h,m,n),8])
                                    if k != h and k != m and h != n:
                                        R.append([Graph55([1,1,1,1,1],i,j,k,h,m,n),2])
                                    if j != h and j != m and j != n:
                                        R.append([Graph56([1,1,1,1,1],i,j,k,h,m,n),120])
        if self._d == 6:
            for i in S:
                for j in S.difference({i}):
                    for k in S.difference({j}):
                        R.append([Graph2([5,1],i,j,k),5])
                        R.append([Graph2([4,2],i,j,k),8])
                        R.append([Graph2([3,3],i,j,k),18])
                        for h in S:
                            if k != h:
                                R.append([Graph31([4,1,1],i,j,k,h),4])
                                R.append([Graph31([1,4,1],i,j,k,h),8])
                                R.append([Graph31([3,2,1],i,j,k,h),6])
                                R.append([Graph31([3,1,2],i,j,k,h),6])
                                R.append([Graph31([1,3,2],i,j,k,h),6])
                                R.append([Graph31([2,2,2],i,j,k,h),16])
                            if j != h:
                                R.append([Graph32([4,1,1],i,j,k,h),8])
                                R.append([Graph32([3,2,1],i,j,k,h),6])
                                R.append([Graph32([2,2,2],i,j,k,h),48])
                            for m in S:
                                if k != h and h != m:
                                    R.append([Graph41([3,1,1,1],i,j,k,h,m),3])
                                    R.append([Graph41([1,3,1,1],i,j,k,h,m),3])
                                    R.append([Graph41([2,2,1,1],i,j,k,h,m),4])
                                    R.append([Graph41([2,1,2,1],i,j,k,h,m),4])
                                    R.append([Graph41([2,1,1,2],i,j,k,h,m),8])
                                    R.append([Graph41([1,2,2,1],i,j,k,h,m),8])
                                if k != h and k != m:
                                    R.append([Graph42([3,1,1,1],i,j,k,h,m),6])
                                    R.append([Graph42([1,3,1,1],i,j,k,h,m),6])
                                    R.append([Graph42([1,1,3,1],i,j,k,h,m),3])
                                    R.append([Graph42([2,2,1,1],i,j,k,h,m),8])
                                    R.append([Graph42([1,1,2,2],i,j,k,h,m),8])
                                    R.append([Graph42([2,1,2,1],i,j,k,h,m),4])
                                    R.append([Graph42([1,2,2,1],i,j,k,h,m),4])
                                if j != h and j != m:
                                    R.append([Graph43([3,1,1,1],i,j,k,h,m),18])
                                    R.append([Graph43([2,2,1,1],i,j,k,h,m),16])
                                for n in S:
                                    if k != h and h != m and m != n:
                                        R.append([Graph51([2,1,1,1,1],i,j,k,h,m,n),2])
                                        R.append([Graph51([1,2,1,1,1],i,j,k,h,m,n),2])
                                        R.append([Graph51([1,1,2,1,1],i,j,k,h,m,n),4])
                                    if k != h and h != m and h != n:
                                        R.append([Graph52([2,1,1,1,1],i,j,k,h,m,n),4])
                                        R.append([Graph52([1,2,1,1,1],i,j,k,h,m,n),4])
                                        R.append([Graph52([1,1,2,1,1],i,j,k,h,m,n),4])
                                        R.append([Graph52([1,1,1,2,1],i,j,k,h,m,n),2])
                                    if k != h and k != m and k != n:
                                        R.append([Graph53([2,1,1,1,1],i,j,k,h,m,n),12])
                                        R.append([Graph53([1,2,1,1,1],i,j,k,h,m,n),12])
                                        R.append([Graph53([1,1,2,1,1],i,j,k,h,m,n),4])
                                    if j != h and h != m and h != n:
                                        R.append([Graph54([2,1,1,1,1],i,j,k,h,m,n),4])
                                        R.append([Graph54([1,1,2,1,1],i,j,k,h,m,n),16])
                                    if k != h and k != m and h != n:
                                        R.append([Graph55([2,1,1,1,1],i,j,k,h,m,n),2])
                                        R.append([Graph55([1,2,1,1,1],i,j,k,h,m,n),2])
                                        R.append([Graph55([1,1,1,2,1],i,j,k,h,m,n),4])
                                    if j != h and j != m and j != n:
                                        R.append([Graph56([2,1,1,1,1],i,j,k,h,m,n),48])
                                    for p in S:
                                        if k != h and h != m and m != n and n != p:
                                            R.append([Graph61([1,1,1,1,1,1],i,j,k,h,m,n,p),2])
                                        if k != h and h != m and m != n and m != p:
                                            R.append([Graph62([1,1,1,1,1,1],i,j,k,h,m,n,p),2])
                                        if k != h and h != m and h != p and m != n:
                                            R.append([Graph63([1,1,1,1,1,1],i,j,k,h,m,n,p),1])
                                        if k != h and h != m and h != n and h != p:
                                            R.append([Graph64([1,1,1,1,1,1],i,j,k,h,m,n,p),6])
                                        if k != h and k != n and k != p and h != m:
                                            R.append([Graph65([1,1,1,1,1,1],i,j,k,h,m,n,p),4])
                                        if k != h and k != p and h != m and h != n:
                                            R.append([Graph66([1,1,1,1,1,1],i,j,k,h,m,n,p),2])
                                        if j != p and k != h and h != m and h != n:
                                            R.append([Graph67([1,1,1,1,1,1],i,j,k,h,m,n,p),8])
                                        if j != p and k != h and k != m and k != n:
                                            R.append([Graph68([1,1,1,1,1,1],i,j,k,h,m,n,p),12])
                                        if k != h and h != n and k != m and m != p:
                                            R.append([Graph69([1,1,1,1,1,1],i,j,k,h,m,n,p),6])
                                        if k != h and k != m and k != n and k != p:
                                            R.append([Graph610([1,1,1,1,1,1],i,j,k,h,m,n,p),24])
                                        if j != h and j != m and j != n and j != p:
                                            R.append([Graph611([1,1,1,1,1,1],i,j,k,h,m,n,p),720])
        return R
...
def lines_on_complete_intersection(l):
    ...
    r = 0
    for p in F:
        s = prod(M.contribution_bundle(p[0],i) for i in l)
        t = M.equivariant_normal_bundle(p[0])
        r = r + s/(p[1]*t)
    return r
